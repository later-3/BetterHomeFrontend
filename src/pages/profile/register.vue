...
const selectedPathText = computed(() => {
  ...
  const community = cascaderOptions.value.find(
    (item) => item.value === communityId
  );
  const building = community?.children?.find(
    (child) => child.value === buildingId
  );
...
async function refreshCascadeData() {
  ...
  try {
    ...
    const buildingMap = buildingList.reduce<Record<string, CascaderNode[]>>(
      (acc, current) => {
        const communityId =
...
async function uploadAvatar() {
  ...
  try {
    ...
    if (uploadRes.statusCode >= 200 && uploadRes.statusCode < 300) {
      ...
      const fileId =
        payload?.data?.id ?? payload?.data?.data?.id ?? payload?.id;
      if (fileId) {
        ...
        <view class="section">
          ...
          <up-cell-group>
            <up-cell
              title="小区 / 楼栋"
              :value="
                selectedPathText || (loadingCascade ? '加载中...' : '请选择')
              "
