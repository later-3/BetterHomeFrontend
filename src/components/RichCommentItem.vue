<template>
  ...
  >
    <div class="comment-avatar" :class="{ 'small-avatar': isReply }">
      <img :src="comment.author.avatar" :alt="comment.author.name" @error="handleAvatarError" />
    </div>
    ...
    <div class="comment-content">
      ...
      <div class="rich-content">
        <template v-for="(segment, index) in parsedSegments" :key="index">
          <p v-if="segment.type === 'text'" class="comment-text">{{ segment.text }}</p>
          <image
          ...
          <video
            ...
            :src="segment.src"
          ></video>
          <AudioPlayer v-else-if="segment.type === 'audio'" :src="segment.src" />
        </template>
      </div>
...
export default {
  ...
  methods: {
    ...
    formatRelativeTime(timestamp) {
      ...
      if (diffInSeconds < 2592000) return `${Math.floor((now - commentTime) / (86400 * 1000))} d`;
