...
# 远端Directus迁移脚本：将billings和billing_payments表更新到v2.5简化设计
# 日期：2025-10-20
# 说明：
#   - 先清空所有数据
#   - billings: 移除paid_amount/status, 改用is_paid布尔字段, 重命名billing_amount为amount
...
echo "=========================================="
echo "远端Directus - 迁移billings和billing_payments表到v2.5"
echo "=========================================="
echo ""
echo "⚠️  警告：此操作将："
...
BILLINGS_COUNT=$(curl -s "$DIRECTUS_URL/items/billings?limit=1&meta=*" \
  ...
  | python3 -c "import sys, json; data = json.load(sys.stdin); print(data.get('meta', {}).get('filter_count', 0))")
...
PAYMENT_IDS=$(curl -s "$DIRECTUS_URL/items/billing_payments?fields=id&limit=1000" \
  -H "Authorization: Bearer $DIRECTUS_TOKEN" \
  | python3 -c "import sys, json; data = json.load(sys.stdin); ids = [item['id'] for item in data.get('data', [])]; print(','.join(ids))")

if [ -n "$PAYMENT_IDS" ]; then
  IFS=',' read -ra ID_ARRAY <<< "$PAYMENT_IDS"
...
BILLING_IDS=$(curl -s "$DIRECTUS_URL/items/billings?fields=id&limit=1000" \
  -H "Authorization: Bearer $DIRECTUS_TOKEN" \
  | python3 -c "import sys, json; data = json.load(sys.stdin); ids = [item['id'] for item in data.get('data', [])]; print(','.join(ids))")
...
echo "   3. cd scripts/test-data && DIRECTUS_URL=https://www.betterhome.ink ./quick-import.sh"
echo ""
