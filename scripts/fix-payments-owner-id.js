...
async function main() {
  console.log('========================================');
  console.log('修复 payments.owner_id');
  console.log('========================================');
  console.log('');
  ...
  try {
    ...
    const currentField = await fetchDirectus('/fields/payments/owner_id');
    console.log('当前配置:');
    ...
    console.log(`  options: ${JSON.stringify(currentField.data.meta.options || null)}`);
    ...
    try {
      ...
      const existingRelation = relations.data.find(r =>
        r.collection === 'payments' && r.field === 'owner_id'
      );
    ...
    const updatedField = await fetchDirectus('/fields/payments/owner_id');
    console.log('更新后配置:');
    ...
    console.log(`  options: ${JSON.stringify(updatedField.data.meta.options || null)}`);

    const isCorrect = updatedField.data.schema.foreign_key_table === 'directus_users' &&
    ...
    console.log('  3. 检查 owner_id 字段是否显示下拉选择器');
    console.log('  4. 确认可以正常选择用户');
    console.log('');
